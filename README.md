...
- `/warn <user> <reason>` - เตือนผู้ใช้และบันทึกประวัติ
- `/warnings <user>` - ดูประวัติการเตือนของผู้ใช้
- `/clearwarn <user> <latest/all>` - ลบการเตือน (ล่าสุด/ทั้งหมด)
- `/setup-muted` - สร้าง Muted role และตั้งค่าสิทธิ์
...
|--------|----------------|
| `/setup`, `/reset`, `/setstream` | Administrator |
| `/send`, `/announce`, `/userinfo`, `/status`, `/history`, `/checkdm`, `/serverinfo`, `/avatar`, `/voicestatus`, `/clear`, `/warnings` | Manage Messages |
| `/timeout`, `/untimeout`, `/mute`, `/unmute`, `/warn`, `/clearwarn` | Moderate Members |
...
### ⚠️ **Warning System** (ใหม่!)
- ระบบเตือนผู้ใช้พร้อมบันทึกประวัติ
- ส่งการแจ้งเตือนไปยัง DM ของผู้ใช้อัตโนมัติ
- ติดตามจำนวนการเตือนสะสม
- ลบการเตือนได้แบบเลือก (รายการล่าสุดหรือทั้งหมด)
- เก็บข้อมูลผู้ดำเนินการและเวลาที่เตือน

### 🎯 **Channel Management** (ปรับปรุง!)
- ล็อค/ปลดล็อคช่องสนทนา
- ตั้งค่า slowmode
- ลบข้อความจำนวนมาก
- คัดลอก emoji จากเซิฟเวอร์อื่น
- จัดการ role และ nickname
...
/warn @user โพสต์ข้อความไม่เหมาะสม     - เตือนผู้ใช้และบันทึกประวัติ
/warnings @user                     - ดูประวัติการเตือนของผู้ใช้
/clearwarn @user latest             - ลบการเตือน
```
...
### การจัดการข้อมูล Warning
- ข้อมูลการเตือนจะถูกเก็บแยกตามเซิฟเวอร์
- แต่ละการเตือนมี ID เฉพาะสำหรับการอ้างอิง
- ระบบ soft delete - การเตือนที่ถูกลบจะไม่แสดงแต่ยังเก็บไว้ใน database
- ข้อมูลรวมถึง: เหตุผล, ผู้ดำเนินการ, เวลา, สถานะ
